...
function [dy,j_ryr]=test_ss_model(y,I_DHPR,bump,coeff)
    % disp(y)
    % Unpack Variables
    Po = y(1); 
    c_ca_lumen = y(2);
    ...
    d_ryr = 2.2 * coeff(2);
    n_ryr = constants.n_ryr * coeff(3);
    j_ryr = n_ryr*Po*d_ryr*(c_ca_lumen-c_ca_ss); 
    
    d_Po = ryr_sobie(Po,c_ca_ss,c_ca_lumen,bump,coeff);
    d_ca_lumen = ca_er_sobie(c_ca_lumen,j_ryr,coeff);
    d_y_ss = ca_ss_sobie(y_ss,j_ryr,I_DHPR,coeff);
    db_dt = d_b_sobie(y_ss,coeff);
    dy = [d_Po; d_ca_lumen; d_y_ss; db_dt(1); db_dt(2); db_dt(3)];
end

function d_Po = ryr_sobie(Po,c_ca_ss,c_ca_lumen,bump,coeff)
    Km_r_max = 19.87 * coeff(11);
    alpha_r = 1.0e-3 * coeff(1); 
    kr_plus_max = 30;
    hill = 4;
    n_ryr = constants.n_ryr * coeff(3);
    EJequiv = 0.1;
    ...
    d_Po = (1-Po)*kr_plus*kcoup^(2*nopen+1-n_ryr)-Po*kr_minus*kcoup^(2*nclosed+1-n_ryr);
end

function d_ca_lumen = ca_er_sobie(c_ca_lumen,j_ryr,coeff)
    CaNSR = 1000;
    tau_refill = 6.5 * coeff(6);
    CSQ = 3e4 * coeff(4);  
    KCSQ = 630 * coeff(10);
    V_ds = 1.0000e-12 * coeff(8);
    V_JSR = 1.6000e-12 * coeff(7);
...
function d_y_ss = ca_ss_sobie(y_ss,j_ryr,I_DHPR,coeff)
    F = 96485 * 1e3; 
    
    V_ds = 1.0000e-12;
    Camyo = 0.1;
    tau_efflux = 1.78e-3 * coeff(5);
    btot = [24 47 900 * coeff(9)]';  
    kp = 1e-3 * [100 115 115]';
    km = 1e-3 * [38 100 1000]'; 
...
function db_dt = d_b_sobie(y_ss, coeff)
    c_ca_ss = y_ss(1);
    b = y_ss(2:end);
    b = b(:);
